func! helpers#plugin_manager#init() abort
  if !filereadable(expand('~/.vim/autoload/plug.vim'))
    return
  endif

  call plug#begin('~/.vim/plugins')
    for l:plugin_file in split(globpath(expand('~/.vim/settings/plugins'), '*.vim'), '\n')
      silent exec 'source ' . l:plugin_file
    endfor
  call plug#end()
endfunc

func! helpers#plugin_manager#is_plugin_loaded(plugin_name, ...) abort
